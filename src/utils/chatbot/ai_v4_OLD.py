...
sys.path.append("../../")
from utils.constants import COLORS_INDEX_PATH, COLORS_PATH, NAMES_INDEX_PATH, NAMES_PATH
...
class AIPlayer:
    def __init__(
        ...
        self.prompter_dict = {
            ...
            "joke": OpenAIPrompter(
                ...
                main_prompt_header=self._update_main_header(
                    ex4.JOKE_MAIN_HEADER,
                ),
                temperature=0.5,
            ),
    ...
    def _update_main_header(self, main_header: str) -> str:
        ...
        return (
            ...
            .replace("CODE_NAME", f"{self.player_state.code_name}")
            ...
            for key, value in example.question.items():
                ...
                if isinstance(value, str):
                    ...
                    value = (
                        value.replace(
                            "NAME",
                            f"{self.player_state.first_name} {self.player_state.last_initial}",
                        )
                        .replace("CODE_NAME", f"{self.player_state.code_name}")
                        .replace("PERSONA", persona_str)
                    )
    ...
    async def choose_action(self, minutes: List[str], chat_log: str) -> str:
        ...
        if action.introduce:
            return await self.generate_action_response("introduce", minutes, chat_log, dcs3.IntroBM)
        elif action.defend:
            ...
            return await self.generate_action_response("joke", minutes, chat_log, dcs3.JokeBM)
            ...
            return await self.generate_action_response("other", minutes, chat_log, dcs3.OtherBM)
        else:
            self.logger.error("Unknown action type chosen.")
            return "Error: Unknown action type."

    async def generate_action_response(
        self, action_type: str, minutes: List[str], chat_log: str, validator: BaseModel
    ) -> str:
    ...
    async def stylize_response(self, response: str, chat_log: str) -> str:
        ...
        input_texts = {"response": response, "examples": "\n".join(self.humans_messages)}

        try:
            response_json = await asyncio.to_thread(prompter.get_completion, input_texts)
            styled_response = dcs3.StylizerBM.model_validate_json(
