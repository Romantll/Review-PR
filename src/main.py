...
from utils.states import ScreenEnum
...
async def main():
    ...
    master_logger = MasterLogger(init=True, clear=False, log_path="./logs/_master.log")
    ...
    while True:
        ...
        if ss in state_handler:
            ...
            if args.debug and ss == ScreenEnum.DEBUG:
                #                              # handler = debug_setup
                next_state, next_gs, next_ps = handler(
                    ss, gs, ps, args.num_players, args.player_number, args.print_prompts
                )
